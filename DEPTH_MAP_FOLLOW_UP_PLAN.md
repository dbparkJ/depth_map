...
## 0. 5분 청크 운영 기준선 (2026-08-25 갱신)

이번 후속 작업에서 Open3D/PDAL 전용 환경, timestamp 기반 5분 청크, 공통 데이터셋 ENU
원점과 단계별 디버그 산출물을 구현했다. 대형 실데이터 정책은 다음처럼 구체화한다.

- 호출 한 번은 청크 하나만 처리하며 기본 검증 길이는 300초다.
- 첫 검증 코스는 기존 `high_density_map`과 같은 `2026-08-19_10-16-33_raw`의 chunk 0이다.
- canonical raw/clean/removed는 전체 선택 점을 보존하고 중간 단계 PLY/BIN/PNG만 상한
  샘플을 사용한다.
- 도로 하부 제거는 `local_surface` 단계의 전체 count, 제거 샘플과 고정 축 top/side
  이미지로 확인한다.
- 파라미터 조정은 보존 raw에 `postprocess_cloud.py`를 최대 한 번씩 적용하며 RGB-D/VO를
  다시 계산하지 않는다.
- 다음 chunk는 현재 chunk의 품질 guard, 10 m 궤적 연속성, 도로 하부 제거와 얇은 구조물
  보존을 확인한 뒤 별도 output에 실행한다. 전체 chunk 자동 반복은 하지 않는다.

청크 0 승인 뒤의 다음 순서는 chunk 1(300–600초) 단독 생성, 두 청크의 공통 원점/경계
불변식 확인, overlap seam과 색상 차이 측정, 그 다음 결합 포맷 설계다. 점군 배열을 한꺼번에
합치는 구현보다 단계 4A의 streaming LAZ 또는 단계 4D의 타일 스트리밍을 우선 검토한다.
